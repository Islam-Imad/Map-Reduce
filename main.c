...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(argv[1], "wordcount") == 0) {
        ...
        init_MapReduceSystem(&mr_sys, number_sum_mapper, number_sum_reducer, MR_DefaultHashPartition, 4, 2, 2);
        printf("Running Number Sum MapReduce...\n");
    }
    else if (strcmp(argv[1], "departmax") == 0) {
        init_MapReduceSystem(&mr_sys, department_max_mapper, department_max_reducer, MR_DefaultHashPartition, 4, 2, 2);
        printf("Running Department Maximum MapReduce...\n");
    }
    else if (strcmp(argv[1], "average") == 0) {
        init_MapReduceSystem(&mr_sys, average_mapper, average_reducer, MR_DefaultHashPartition, 4, 2, 2);
